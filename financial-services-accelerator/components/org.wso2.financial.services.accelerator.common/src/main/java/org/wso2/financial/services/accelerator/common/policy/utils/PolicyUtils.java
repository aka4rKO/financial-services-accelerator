...
public class PolicyUtils {
    ...
    /**
     * Method to invoke external service call with basic auth.
     * @param propertyMap
     * @param payload
     * @return
     */
    public static JSONObject invokeExternalServiceCall(Map<String, Object> propertyMap, String payload) {
        try {
            HttpPost httpPost = new HttpPost(propertyMap.get("endpoint").toString());
            ...
            Map<String, String> securityData = (Map<String, String>) propertyMap.get("security");
            if ("basic-auth".equals(securityData.get("type"))) {
                setBasicAuthHeader(httpPost, securityData.get("username"), securityData.get("password"));
            } else if ("oauth2".equals(securityData.get("type"))) {
                setOauth2AuthHeader(httpPost, securityData.get("tokenEP"), securityData.get("clientId"),
                        securityData.get("clientSecret"));
            }
